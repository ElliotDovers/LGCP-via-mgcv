...
fit_gam_opt <- function(data, quad, pred, n_knots, which.cov.fn = 2, crit.method = c("REML", "GCV.Cp"), tolerance = 2){
  ...
  fit.new.score <- function(logrho) {
    # tmp.m <- gam(response ~ env + te(x,y, bs = "gp", k = n_knots, m = list(c(which.cov.fn, exp(logrho), 1)), d = 2), data=tmp.dat, family=poisson(), weights=p.wt, method=crit.method)
    tmp.m <- gam(response ~ env + s(x,y, bs = "gp", k = n_knots, m = c(which.cov.fn, exp(logrho), 1)), data=tmp.dat, family=poisson(), weights=p.wt, method=crit.method)
  ...
  time2B <- system.time(assign("m2", gam(response ~ env + s(x,y, bs = "gp", k = n_knots, m = c(which.cov.fn, exp(opt2$minimum), 1)), data = tmp.dat, family = poisson(), weights = p.wt, method=crit.method)))
  ...
  COVER_BETA <- BETA_HAT + qnorm(0.025) * BETA_SE  <= attr(pres, "sim.info")$Beta.env & BETA_HAT + qnorm(0.975) * BETA_SE >= attr(pres, "sim.info")$Beta.env
  # calculate the rmse in range parameter estimate
  SQER_RHO <- (m2$smooth[[1]]$gp.defn[2] - attr(pres, "sim.info")$latent.practical.range)^2
  # calculate coverage for range parameter estimate
  COVER_RHO <- NA
  e2 <- data.frame(GP_APPROX = class(m2$smooth[[1]])[1], COV_FN = m2$smooth[[1]]$gp.defn[1], POW = m2$smooth[[1]]$gp.defn[3],
                   K = m2$smooth[[1]]$bs.dim, FIT = "mgcv", KL = KLdiv, MAE = MAE,
                   SQER_BETA = SQER_BETA, COVER_BETA = COVER_BETA, BETA_HAT = BETA_HAT,
                   SQER_RHO = SQER_RHO, COVER_RHO = COVER_RHO, RHO_HAT = m2$smooth[[1]]$gp.defn[2],
