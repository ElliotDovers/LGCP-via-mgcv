...
fit_scampr_fixed <- function(data, quad) {
  
  # bfs <<- glmmTMB::make_basis(n_knots, quad)
  # class(bfs)[2] <- "bf.df"
  ...
  m_va <- scampr(formula = present ~ env, data = tmp.dat, basis.functions = bfs, sre.approx = "variational", starting.pars = m0)
  m_lp <- scampr(formula = present ~ env, data = tmp.dat, basis.functions = bfs, sre.approx = "laplace", starting.pars = m0)
  ...
  COVER_RHO <- c(NA, NA)
  e0 <- data.frame(GP_APPROX = c("FIXED", "FIXED"), COV_FN = c(NA, NA), POW = c(NA, NA),
                   K = c(nrow(bfs), nrow(bfs)), FIT = rep("scampr", 2), KL = c(KLdiv_va, KLdiv_lp), MAE = c(MAE_va, MAE_lp),
                   ...
                   TIME = c(m_va$cpu["opt"] + m0$cpu["opt"], m_lp$cpu["opt"] + m0$cpu["opt"]), DEF_KNOT_LOC = c(T, T), INLA_K = c(if (is.null(m_va$basis.functions)) {0} else {nrow(m_va$basis.functions)}, if (is.null(m_lp$basis.functions)) {0} else {nrow(m_lp$basis.functions)}), CRIT = c(logLik(m_va), logLik(m_lp)), EDF = c(NA, NA), method = c("VA", "LP")
  )
